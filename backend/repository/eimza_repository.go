...
func (r *EimzaRepository) GetPendingSignatures(uyeID int, rol string) ([]models.Proje, error) {
	...
	for _, singleRol := range roles {
		...
		case "akademisyen":
			...
			query = `
				SELECT p.proje_id, COALESCE(p.proje_kodu, ''), COALESCE((SELECT baslik FROM proje_baslik WHERE proje_id = p.proje_id AND dil_kodu = 'tr'), ''), COALESCE((SELECT baslik FROM proje_baslik WHERE proje_id = p.proje_id AND dil_kodu = 'en'), ''), COALESCE(p.sure_ay, 0), COALESCE((SELECT COALESCE(SUM(toplam_fiyat), 0) FROM proje_butce WHERE proje_id = p.proje_id), 0), p.olusturma_tarihi,
...
func (r *EimzaRepository) GetSignedDocuments(uyeID int) ([]models.Proje, error) {
	query := `
		SELECT p.proje_id, COALESCE(p.proje_kodu, ''), COALESCE((SELECT baslik FROM proje_baslik WHERE proje_id = p.proje_id AND dil_kodu = 'tr'), ''), COALESCE((SELECT baslik FROM proje_baslik WHERE proje_id = p.proje_id AND dil_kodu = 'en'), ''), COALESCE(p.sure_ay, 0), COALESCE((SELECT COALESCE(SUM(toplam_fiyat), 0) FROM proje_butce WHERE proje_id = p.proje_id), 0), pi.imza_tarihi,
		       COALESCE(pd.durum_adi, 'taslak'), COALESCE(pbt.bap_turu, 'Münferit'),
		       COALESCE(u.ad || ' ' || u.soyad, '') as koordinator_ad_soyad
		FROM proje p
